...
| Shared single-user credential | Actions cannot be attributed to separate operators. | Signed HTTP-only session cookie; username/password and session secret are configurable by environment. | Must add users, roles, audit trail, CSRF protection, and password lifecycle before real multi-user operation. |
