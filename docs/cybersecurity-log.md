...
| --- | --- | --- | --- |
| Accidental public exposure | Local prototype could be accessed by unintended users if bound to all interfaces. | Local default binds to `127.0.0.1`; Docker uses `0.0.0.0` only inside an explicit container demo on port `8050`; UI/API require login. | Production deployment needs HTTPS and network policy. |
| User and role misconfiguration | A bad role assignment could expose administrative actions. | SQLite users, `admin`/`tecnico`/`dispatcher` roles, admin-only routes, signed HTTP-only session cookie, and last-active-admin protection. | Add audit trail, CSRF protection, and password lifecycle before real multi-user operation. |
